...
### 问题3：读取超时 (ReadTimeoutError)

**症状**：日志频繁出现 `ReadTimeoutError: Read timed out`
...
**解决方案**：
- 降低并发数：修改脚本中的 `MAX_WORKERS=5`（默认 20）
- 脚本已内置重试机制，会自动重试失败的地址
- 在 FortiGate 低峰时段执行更新

#### 情况2：更新地址组时超时（更常见）
当地址组包含 8000+ 条记录时，FortiGate 处理大 JSON 负载需要很长时间。

**根本原因**：
- 每批更新需要先 GET 当前所有成员（8000+ 条），再 PUT 追加新成员
- 随着地址组变大，FortiGate 的验证和索引时间呈指数级增长
- 这是 FortiGate API 的性能瓶颈，与运行脚本的机器性能无关
...
**如果仍然超时**：
1. 进一步降低批次大小（修改脚本第 510 行 `batch_size = 100`）
2. 选择业务最低峰时段执行
3. 考虑升级 FortiGate 固件或硬件
4. 查看详细的 [TROUBLESHOOTING.md](./china-only/TROUBLESHOOTING.md) 文档
