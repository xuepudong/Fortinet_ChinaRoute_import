#!/usr/bin/env python3
"""
飞塔防火墙中国IP地址自动更新脚本（ALL-CN 精简版）
基于苍狼IP库，每天自动更新中国大陆全部IP地址段（仅维护 ALL-CN 地址组）
不再创建移动/联通/电信的独立地址组
"""
...
if os.environ.get('GROUP_NAME'):
    CONFIG['GROUPS']['ALL-CN'] = os.environ['GROUP_NAME']
if os.environ.get('IP_SOURCE_URL'):
    CONFIG['IP_SOURCES']['ALL-CN'] = os.environ['IP_SOURCE_URL']
...
class FortiGateIPUpdater:
    ...
    def update_single_isp(self, isp_key):
        ...
        # 4. 创建新地址对象
        # 地址对象命名格式: ALL-CN_1.2.3.0_24 (斜杠替换为下划线)
        addresses = []
    ...
    def update_all(self):
        """更新 ALL-CN IP 地址组"""
        self.logger.info("\n" + "="*80)
        self.logger.info("飞塔防火墙中国IP地址更新任务开始 (ALL-CN)")
        self.logger.info("="*80)
        ...
        # 2. 更新 ALL-CN IP
        results = []
        for isp_key in self.config['IP_SOURCES'].keys():
...
"""
        ...
        self.notifier.send("飞塔防火墙IP更新完成 (ALL-CN)", content)
...
def main():
    """主函数"""
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='飞塔防火墙中国IP地址自动更新服务（仅 ALL-CN 地址组）')
    ...
    print("="*80)
    print("飞塔防火墙中国IP地址自动更新服务 (ALL-CN 精简版)")
    print("="*80)
    print(f"地址组: {CONFIG['GROUPS']['ALL-CN']}")
